...
kpi_sale.metric("Sales", cur_kpi_dict['sales'], f"{np.random.randint(-2,6)}%")
kpi_num_order.metric("Orders", cur_kpi_dict['num_order'], f"{np.random.randint(-2,6)}%")
kpi_num_cust.metric("Distinct Customers", cur_kpi_dict['num_cust'], f"{np.random.randint(-2,6)}%")
...
with tab_product:
    ...
    with surging_product:
        ...
        if surging_items_df is not None:
            st.dataframe(
                surging_items_df,
